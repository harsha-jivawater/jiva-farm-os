...
comment on column public.devices.deleted_by_user_id is
  'Internal Admin user who removed an unused warehouse device from active inventory.';
comment on column public.devices.deletion_reason is
  'Required business reason for removing an unused warehouse device from active inventory.';
...
begin
  if old.deleted_at is null and new.deleted_at is not null then
    ...
    if old.current_holder_type::text <> 'Warehouse'
      ...
      or exists (select 1 from public.institution_sale_order_items oi where oi.assigned_device_id = old.id)
...
drop trigger if exists devices_guard_unused_stock_deletion on public.devices;
create trigger devices_guard_unused_stock_deletion
before update of deleted_at on public.devices
for each row
when (old.deleted_at is distinct from new.deleted_at)
execute function private.guard_unused_stock_device_deletion();
