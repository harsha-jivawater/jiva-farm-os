-- Focused page KPI summary RPCs.
-- These functions are SECURITY INVOKER so normal Supabase RLS remains active.
-- They return only aggregate counts and do not modify data.
...
as $$
  with scoped as (
    ...
    where (
      ...
      and (nullif(p_state, '') is null or fl.state = p_state)
      and (nullif(p_district, '') is null or fl.district = p_district)
...
as $$
  with scoped as (
    ...
    where i.deleted_at is null
      ...
      and (nullif(p_state, '') is null or i.state = p_state)
      and (nullif(p_district, '') is null or i.district = p_district)
...
as $$
  with scoped as (
    ...
    where i.deleted_at is null
      ...
      and (nullif(p_primary_state, '') is null or i.primary_state = p_primary_state)
...
as $$
  with scoped as (
    ...
    where p.deleted_at is null
      ...
      and (nullif(p_state, '') is null or p.state = p_state)
      and (nullif(p_district, '') is null or p.district = p_district)
