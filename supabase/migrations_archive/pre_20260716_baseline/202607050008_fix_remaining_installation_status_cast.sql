  ...
  return (
    ...
    matched_leads as (
      ...
      where (p_state is null or l.state = p_state)
        ...
        and (p_crop is null or l.primary_crop::text = p_crop)
        and (p_product_model is null or l.product_recommended::text = p_product_model)
    ),
    matched_dealers as (
      select d.*
    ...
    matched_pilots as (
      ...
      where (p_state is null or p.state = p_state)
        ...
        and (p_crop is null or p.crop::text = p_crop)
        and (p_product_model is null or p.product_model::text = p_product_model)
    ),
    matched_installations as (
      ...
      where (p_state is null or i.state = p_state or l.state = p_state or p.state = p_state)
        ...
        and (p_crop is null or l.primary_crop::text = p_crop or p.crop::text = p_crop)
        and (p_product_model is null or i.product_model::text = p_product_model or p.product_model::text = p_product_model)
    ),
    matched_devices as (
      select d.*
      ...
      where (
        ...
        and (
          p_crop is null
          or l.primary_crop::text = p_crop
          or p.crop::text = p_crop
          or exists (
            select 1
        ...
        and (p_product_model is null or d.product_model::text = p_product_model)
    ),
    matched_dispatches as (
      ...
      where (
        ...
        and (p_product_model is null or dp.product_model::text = p_product_model or p.product_model::text = p_product_model)
    ),
    ...
    fy_installations as (
      ...
      where installation_status::text in (select status_text from installed_statuses)
    ...
    rsm_rows as (
      select
        ...
        count(distinct rl.id) filter (where rl.sales_completed is true or rl.lead_status::text = 'Won') as sales,
        count(distinct fi.id) filter (
          where fi.dealer_id is not null
             or fi.installation_type::text = 'Dealer Farmer Installation'
    ...
    installations_by_month as (
      ...
      left join range_installations ri
        on ri.installation_status::text in (select status_text from installed_statuses)
       and date_trunc('month', ri.installation_date)::date = m.month_start
      group by m.month_start
      order by m.month_start
    ),
    installations_by_product as (
      select coalesce(product_model::text, 'Not set') as label, count(*)::int as value
      from range_installations
      where installation_status::text in (select status_text from installed_statuses)
      group by coalesce(product_model::text, 'Not set')
      order by count(*) desc, coalesce(product_model::text, 'Not set')
    ),
    leads_by_funnel_stage as (
      select coalesce(funnel_stage::text, 'Not set') as label, count(*)::int as value
      from range_leads
      group by coalesce(funnel_stage::text, 'Not set')
      order by count(*) desc, coalesce(funnel_stage::text, 'Not set')
    ),
    devices_by_status as (
      select coalesce(device_status::text, 'Not set') as label, count(*)::int as value
      from matched_devices
      group by coalesce(device_status::text, 'Not set')
    ),
    pilots_by_status as (
      select coalesce(pilot_status::text, 'Not set') as label, count(*)::int as value
      from range_pilots
      group by coalesce(pilot_status::text, 'Not set')
      order by count(*) desc, coalesce(pilot_status::text, 'Not set')
    ),
    ...
    agronomist_rows as (
      select
        u.id,
        u.full_name as name,
        count(distinct p.id) filter (where p.pilot_status::text in (select status from active_pilot_statuses)) as active_pilots,
        count(distinct v.id) filter (where v.visit_status::text = 'Completed') as visits_completed,
        count(distinct r.id) filter (where r.report_status::text = 'Submitted') as reports_submitted,
        count(distinct p.id) filter (
          where p.scale_up_recommended is true
             or p.pilot_status::text = 'Scale-up Recommended'
    ...
    select jsonb_build_object(
      ...
      'management', jsonb_build_object(
        ...
        'warehouseStock', (select count(*) from matched_devices where device_status::text = 'In Warehouse'),
        'dealerStock', (select count(*) from matched_devices where device_status::text = 'With Dealer' or current_holder_type::text = 'Dealer'),
        'activePilots', (select count(*) from matched_pilots where pilot_status::text in (select status from active_pilot_statuses)),
        'activeDealers', (select count(*) from matched_dealers where dealer_status::text = 'Active Dealer'),
        'activeInstitutionalPartners', (
          select count(*)
          from matched_institutions
          where institution_status::text in (select status from active_institution_statuses)
             or scale_up_status::text = 'Active Scale-up'
        )
      ),
      'sales', jsonb_build_object(
        'newLeadsThisMonth', (select count(*) from month_leads),
        'openLeads', (select count(*) from range_leads where lead_status::text = 'Open'),
        'wonLeads', (select count(*) from range_leads where lead_status::text = 'Won'),
        'lostLeads', (select count(*) from range_leads where lead_status::text = 'Lost'),
        ...
        'followupsCompleted', (
          ...
          where followup_completed is true
             or funnel_stage::text = '15-Day Follow-up Completed'
        ),
        'followupsDue', (
          select count(*)
          from range_followups
          where followup_status::text = 'Due'
      ...
      'dealers', jsonb_build_object(
        ...
        'activeDealers', (select count(*) from matched_dealers where dealer_status::text = 'Active Dealer'),
        'dormantDealers', (select count(*) from matched_dealers where dealer_status::text = 'Dormant Dealer'),
        'trainedDealers', (select count(*) from matched_dealers where training_status::text = 'Training Completed'),
        'dealerStock', (select count(*) from matched_devices where device_status::text = 'With Dealer' or current_holder_type::text = 'Dealer'),
        'dealerInstallations', (
          select count(*)
          from range_installations
          where dealer_id is not null
             or installation_type::text = 'Dealer Farmer Installation'
        ),
        'monthlyInstallations', (select count(*) from month_installations),
      ...
      'institutions', jsonb_build_object(
        ...
        'pilotProposalsShared', (
          select count(*)
          from matched_institutions
          where proposal_shared::text = 'Yes'
             or institution_status::text = 'Pilot Proposal Shared'
        ),
        'institutionalPilotsStarted', (select count(*) from matched_pilots where institution_id is not null),
        'scaleUpOpportunities', (
          select count(*)
          from matched_institutions
          where scale_up_status::text in (select status from scale_up_statuses)
      ...
      'pilots', jsonb_build_object(
        ...
        'activePilotsInRange', (select count(*) from range_pilots where pilot_status::text in (select status from active_pilot_statuses)),
        'pilotVisitsCompleted', (select count(*) from range_visits where visit_status::text = 'Completed'),
        'visitReportsSubmitted', (select count(*) from range_reports where report_status::text = 'Submitted'),
        'finalPilotReportsApproved', (
          select count(*)
          from range_reports
          where report_type::text = 'Final Pilot Report'
            and report_status::text = 'Approved'
        ),
        'reportsPending', (
          select count(*)
          from range_reports
          where report_status::text in ('Draft', 'Revision Required')
        ),
        'scaleUpRecommendedPilots', (
          select count(*)
          from matched_pilots
          where scale_up_recommended is true
             or pilot_status::text = 'Scale-up Recommended'
        ),
        'closedSuccessfulPilots', (select count(*) from matched_pilots where pilot_status::text = 'Closed - Successful')
      ),
      'agronomist', jsonb_build_object(
        'activeOwnPilots', (
          select count(*)
          from current_agronomist_pilots
          where pilot_status::text in (select status from active_pilot_statuses)
        ),
        'activeTeamPilots', (
          select count(*)
        ...
        'visitsCompleted', (
          ...
          left join current_agronomist_team_pilots p
            on p.id = v.pilot_id
          where v.visit_status::text = 'Completed'
            and (
              p.id is not null
        ...
        'reportsSubmitted', (
          ...
          left join current_agronomist_team_pilots p
            on p.id = r.pilot_id
          where r.report_status::text = 'Submitted'
            and (
              p.id is not null
        ...
        'finalReportsPending', (
          select count(*)
          from current_agronomist_team_pilots
          where pilot_status::text = 'Final Report Pending'
        ),
        'scaleUpRecommended', (
          select count(*)
          from current_agronomist_team_pilots
      ...
      'researchAssistant', jsonb_build_object(
        ...
        'followupsCompleted', (
          ...
          where followup_owner_user_id = public.get_current_user_id()
            and followup_status::text = 'Completed'
        )
      ),
      'rdHead', jsonb_build_object(
        'totalActivePilots', (select count(*) from matched_pilots where pilot_status::text in (select status from active_pilot_statuses)),
      ...
      'stock', jsonb_build_object(
        ...
        'warehouse', (select count(*) from matched_devices where device_status::text = 'In Warehouse'),
        'dealer', (select count(*) from matched_devices where device_status::text = 'With Dealer' or current_holder_type::text = 'Dealer'),
        'dispatched', (select count(*) from range_dispatches where dispatch_status::text = 'Dispatched'),
        'installedFarmer', (select count(*) from matched_devices where device_status::text = 'Installed at Farmer Site'),
        'installedPilot', (select count(*) from matched_devices where device_status::text = 'Installed for Pilot'),
        'returned', (select count(*) from matched_devices where device_status::text = 'Returned'),
        'damagedHold', (select count(*) from matched_devices where device_status::text in ('Damaged', 'Hold'))
      ),
      'rsmRows', coalesce((
        select jsonb_agg(jsonb_build_object(
          'id', id,
