...
async function SalesTargetsContent() {
  ...
  await getCurrentInternalUser(supabase, "/sales/targets");
  const { data: users } = await supabase.from("users").select("id,full_name,role").eq("is_active", true).in("role", ["RSM", "Sales Head"]).order("full_name");
  ...
  return (
    <section>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[360px_1fr]">
        <form action={createSalesTargetAction} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
